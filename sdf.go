...
type SDFHandle struct {
	ctx              *base.Ctx
	deviceHandle     base.DeviceHandle
...
func New(lib string, sessionCacheSize int) (*SDFHandle, error) {
	...
	if ctx == nil {
		...
		slog.Debug(fmt.Sprintf("lib[%s] invalid, use HSM_LIB[%s] from env\n", lib, libEnv))
		ctx = base.New(libEnv)
		if ctx == nil {
	...
	if sessionCacheSize <= 0 {
		...
		if err == nil && sessionSize > 0 {
			slog.Debug(fmt.Sprintf("sessionCacheSize[%d] invalid, use HSM_SESSION_CACHE_SIZE[%s] from env\n",
				sessionCacheSize, sessionSizeStr))
			sessionCacheSize = sessionSize
		} else {
			slog.Debug(fmt.Sprintf("sessionCacheSize[%d] and HSM_SESSION_CACHE_SIZE[%s] invalid, use default size[%d]\n",
	...
	var err error
	var deviceHandle base.DeviceHandle
	for i := 0; i < 3; i++ {
...
func (h *SDFHandle) withSession(f func(device base.DeviceHandle, session base.SessionHandle) error) error {
	session, err := h.getSession()
	if err != nil {
		return err
	}
	defer h.returnSession(err, session)
	return f(h.deviceHandle, session)
}
